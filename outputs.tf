...
# -------------------------------------------------------------------
# Debug: Show raw VNET discovery results during terraform plan
# -------------------------------------------------------------------

output "vnet_discovery" {
  value = {
    for region, ds in data.azurerm_resources.vnet_by_tag : region => {
      matches = length(ds.resources)
  ...
  description = "Raw VNET discovery results from azurerm_resources"
}
...
output "regional_private_endpoints" {
  value = {
    for region, endpoint in mongodbatlas_privatelink_endpoint.atlas : region => {
      ...
      azure_resource_group = local.pl_regions[region].resource_group
...
output "regional_summary" {
  value = {
    for region, config in local.pl_regions : region => {
